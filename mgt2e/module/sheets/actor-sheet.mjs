...
export class MgT2ActorSheet extends foundry.appv1.sheets.ActorSheet {
    ...
    async getData() {
        ...
        context.currentYear = game.settings.get("mgt2e-piggy", "currentYear");
        ...
        if (type === 'traveller' || type === 'package') {
            ...
            if (actorData.settings.autoAge) {
                actorData.sophont.age = parseInt(game.settings.get("mgt2e-piggy", "currentYear")) - actorData.birthYear;
            }
    ...
    _prepareSpacecraftItems(context) {
        ...
        if (context.actor.getFlag("mgt2e-piggy", "damage_armour")) {
            let armourDamage = context.actor.getFlag("mgt2e-piggy", "damage_armour");
    ...
    async _prepareItems(context) {
        ...
        if (!this.actor.encLock) {
            ...
            try {
                if (game.settings.get("mgt2e-piggy", "useEncumbrance")) {
  ...
  /* -------------------------------------------- */
    ...
    _useReaction(actor) {
        ...
        if (!effect) {
            ...
            if (effect?.flags?.["mgt2e-piggy"]?.effect) {
                let value = parseInt(effect.flags["mgt2e-piggy"].value);
                value -= 1;
                effect.setFlag("mgt2e-piggy", "value", value);
            } else {
                effect.setFlag("mgt2e-piggy", "value", -1);
            }
        }
    }
    ...
    _clearDodge(actor) {
        actor.unsetFlag("mgt2e-piggy", "reaction");
    }
    ...
    async _clearStunned(actor) {
        ...
        await actor.unsetFlag("mgt2e-piggy", "stunned");
        await actor.unsetFlag("mgt2e-piggy", "stunnedRounds");
    }

    _clearStatus(actor, status) {
        actor.unsetFlag("mgt2e-piggy", status);
        let e = actor.effects.find(e => e?.flags?.["mgt2e-piggy"]?.effect === status);
        if (e) {
            e.delete();
        }
    }
    ...
    _modifyStatus(actor, status, modifier) {
        ...
        if (e) {
            ...
            let value = parseInt(e.getFlag("mgt2e-piggy", "value"));
            value += modifier;
            ...
            e.setFlag("mgt2e-piggy", "value", value);
            if (!config.mono) {
                if (value < 0) {
                    e.setFlag("mgt2e-piggy", "css", "statusWarn");
                } else if (value > 0) {
                    e.setFlag("mgt2e-piggy", "css", "statusGood");
                }
            }
    ...
    async _onSubmit(event, updateData, preventClose, preventRender) {
        ...
        if (this.actor.type === "spacecraft" && game.settings.get("mgt2e-piggy", "autoResizeSpacecraft")) {
            let size = 1;
